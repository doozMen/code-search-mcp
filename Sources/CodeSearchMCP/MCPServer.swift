...
actor MCPServer: Sendable {
    ...
    init(
        ...
        self.logger.info("MCP server initialized", metadata: [
            "index_path": "\(indexPath)"
        ])

        // Index initial projects if provided
        if !projectPaths.isEmpty {
            self.logger.info("Indexing projects", metadata: [
                "count": "\(projectPaths.count)"
            ])
            for projectPath in projectPaths {
                do {
                    try await self.projectIndexer.indexProject(path: projectPath)
                    self.logger.info("Project indexed", metadata: [
                        "path": "\(projectPath)"
                    ])
                } catch {
                    self.logger.warning("Failed to index project", metadata: [
                        "path": "\(projectPath)",
                        "error": "\(error)"
                    ])
                }
