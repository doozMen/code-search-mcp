...
actor EmbeddingService: Sendable {
    ...
    init(indexPath: String) {
        ...
        logger.debug("EmbeddingService initialized", metadata: [
    ...
    func generateEmbedding(for text: String) async throws -> [Float] {
        ...
        logger.debug("Generating new embedding", metadata: [
    ...
    private func getCachedEmbedding(for text: String) throws -> [Float]? {
        ...
        do {
            ...
            logger.debug("Retrieved cached embedding", metadata: [
                "size": "\(embedding.count)"
            ])
            return embedding
        } catch {
            logger.warning("Failed to decode cached embedding", metadata: [
                "error": "\(error)"
            ])
            return nil
        }
    ...
    private func cacheEmbedding(_ embedding: [Float], for text: String) async throws {
        ...
        do {
            ...
            logger.debug("Cached embedding", metadata: [
                "hash": "\(textHash)",
                "size": "\(embedding.count)"
            ])
        } catch {
            logger.warning("Failed to cache embedding", metadata: [
                "error": "\(error)"
            ])
    ...
    func clearCache() async throws {
        do {
            ...
            logger.error("Failed to clear embedding cache", metadata: [
                "error": "\(error)"
            ])
